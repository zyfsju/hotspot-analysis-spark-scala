...
The purpose of the project is to analyze the spatial hot zones and hot cells for a dataset of monthly taxi pick up locations in New York, with Apache Spark and Scala.

## Development Environment Setup

The project requires Apache Spark 2.4.7 pre-built for Apache Hadoop 2.7, scala, open JDK 8 and sbt on a Linux machine. I created the virtual environment containing these packages with a Dockerfile, and used it as the base image for Spark service and my project application. I defined these containers and their dependencies in a docker-compose.yml file, and mounted the project folder as volumes to the project application container, so that every time a change is made, I can re-build the project and submit the compiled .jar file to Spark, with a simple docker-compose up command in the terminal.

Run `./docker-script.sh` to spin up the containers and submit a job. Specifically, the following commands can be excuted to
...
The command to submit Hotzone job after generating the jar file is shown below. Note I set the project directory to `/app` in the Docker container, and Spark is installed in `/spark` directory.
...
```
The output CSV file with all zones and their hotness is stored under `test/output0`.
...
The command to submit Hotcell job after generating the jar file is shown below.
```
...
```
The output CSV file with the coordinates of the top hottest 50 cells is stored under `test/output0`.
